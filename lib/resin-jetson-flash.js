...
module.exports = class ResinJetsonFlash {
	constructor(deviceType, image, odmdata, assetDir, output, cache) {
		...
		this.dictionary = {
			...
			/* The AVerMedia D315 is a custom carrier board for the AGX Orin SoM, so it
			 * is flashed exactly like the AGX Orin Devkit 32GB: the stock NVIDIA BSP
			 * with the carrier-board-specific files from the AVerMedia BSP copied in.
			 * boardConf is the flash.sh board configuration shipped by AVerMedia.
			 */
			['avermedia-d315-agx-orin-32gb']: {
				url:
					'https://developer.nvidia.com/downloads/embedded/l4t/r36_release_v5.0/release/Jetson_Linux_r36.5.0_aarch64.tbz2',
				partitions: {
					'resin-boot': { path: null },
                ...
                medium: 'sdmmc_user',
                boardConf: 'jetson-agx-orin-d315ao',
                avermediaD315: true,
            },
	...
	/* Both the AGX Orin Devkit 32GB and the AVerMedia D315 carrier board use the
	 * same SoM and the same L4T BSP, so they need the same BSP fixups.
	 */
	isAgxOrin() {
		return (
			this.deviceType.toString() === 'jetson-agx-orin-devkit' ||
			this.dictionary[this.deviceType].avermediaD315 === true
		);
	}

	/* Copy the AVerMedia D315 carrier-board files over the stock NVIDIA BSP tree.
	 * jetson-agx-orin-d315ao.conf sources jetson-agx-orin-devkit.conf and only
	 * overrides the device tree, picked at flash time from the module SKU read
	 * off the SoM EEPROM.
	 *
	 * p3701.conf.common is deliberately not copied: the one shipped by AVerMedia
	 * carries no D315-specific change, so the stock file of the BSP release we
	 * download is used rather than mixing in one from another L4T release.
	 */
	injectAvermediaD315Bsp(workPath) {
		const bspDir = path.resolve(__dirname, '../Orin_Flash/avermedia-d315');
		const files = [
			['jetson-agx-orin-d315ao.conf', '.'],
			['tegra234-p3737-0000+p3701-0000-nv-d315.dtb', 'kernel/dtb'],
			['tegra234-p3737-0000+p3701-0004-nv-d315.dtb', 'kernel/dtb'],
			['tegra234-p3737-0000+p3701-0005-nv-d315.dtb', 'kernel/dtb'],
			['tegra234-mb1-bct-pinmux-p3701-0000-a04.dtsi', 'bootloader/generic/BCT'],
			['tegra234-mb1-bct-gpio-p3701-0000-a04.dtsi', 'bootloader'],
		];

		console.log('Injecting AVerMedia D315 BSP files into the L4T tree');
		for (const [file, destination] of files) {
			fs.copyFileSync(join(bspDir, file), join(workPath, destination, file));
		}
	}
	...
	spawnFlash() {
		return new Bluebird(async (resolve, reject) => {
			...
			if (this.isAgxOrin()) {
					...
					if (this.dictionary[this.deviceType].avermediaD315) {
						this.injectAvermediaD315Bsp(workPath);
					}
			}
			...
			const boardConf =
				this.dictionary[this.deviceType].boardConf || this.deviceType;
